...
public class SceneManager : MonoBehaviour
{
    ...
    public void LoadScene(int sceneNumber, int loadingImage)
    {
        StartCoroutine(LoadingRoutine(sceneNumber, loadingImage));
    }
    ...
    IEnumerator LoadingRoutine(int sceneNumber, int loadingImage)
    {
        loadingUI.SetProgress(0f);
        loadingUI.FadeOut();
        loadingUI.SetImage(loadingImage);
        yield return new WaitForSeconds(0.5f);
        Time.timeScale = 0f;
        AsyncOperation oper = UnitySceneManager.LoadSceneAsync(sceneNumber);
        while (!oper.isDone)
        {
            loadingUI.SetProgress(Mathf.Lerp(0.0f, 0.5f, oper.progress));
            yield return null;
